...
def process_registration_data(file_path, department_map, hall_patterns):
    # Reset Database
    def reset_database():
    ...
    # Step 2: Handle the input file (Excel or CSV)
    ...
    # Step 3: Allocate seats based on the fetched registration data
    allocations = allocate_seats(register_numbers, hall_patterns)

    # Step 4: Display the seat allocations for each hall
    for hall_name, pattern in hall_patterns.items():
        print(f"\n{hall_name} Allocation:")
        for row in allocations[hall_name]:
            print("  " + " ".join(row))

# Hall Patterns (provided in your initial code)
hall_patterns = {
    "HALL_1": [
        ["1", "3", "2", "3", "1"],
        ["2", "4", "1", "4", "2"],
...
# Department Mapping (as provided in your code)
department_map = {1: "CSE", 2: "MECH", 3: "AIDS", 4: "CSBS"}

# Example usage
file_path = input("Enter the file path (Excel or CSV): ").strip()
process_registration_data(file_path, department_map, hall_patterns)